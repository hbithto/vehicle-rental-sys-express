...
export const startBookingReturnCron = () => {
    ...
    cron.schedule('0 0 * * *', async () => {
        ...
        try {
            ...
            await client.query('ROLLBACK');
            logger.error('[CRON] Error processing overdue bookings:', error);
        }
    }, {
        name: 'booking-return-cron',
        timezone: 'Asia/Dhaka'
    });

    logger.info('[CRON] Booking return cron job initialized.');
};
